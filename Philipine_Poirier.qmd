...
Here is the link for the [github project](https://github.com/philipine1234/grades.git)
...
oldest <- students %>%
  group_by(group) %>%
  filter(age == max(age)) %>%   # garder seulement le(s) plus âgé(s) du groupe
...
# Question 12
```{r}
#add semesters to the table
grades_sem <- grades %>%
  left_join(courses %>% select(course_id, semester),
            by = "course_id")
#not sure if we needed to do it for the earth course only or for the all the courses. 
grades_sem %>%
  ggplot(aes(x = factor(semester), y = grade)) +
  geom_boxplot()
```
...
age_success <- pass %>%
  left_join(students %>%
              select(id, age), by = "id") %>%
  group_by(age) %>%
  summarise(success_rate = mean(pass == TRUE))
